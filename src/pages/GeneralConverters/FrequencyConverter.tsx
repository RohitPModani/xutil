...
function FrequencyConverter() {
  ...
  const convertFrequency = (value: number, unit: string): Record<FrequencyUnit, number> => {
    // Validate input unit
    if(!Object.keys(UNIT_TO_HERTZ).includes(unit)) {
      throw new Error(`Invalid unit: ${unit}. Supported units: ${Object.keys(UNIT_TO_HERTZ).join(', ')}`);
    }
