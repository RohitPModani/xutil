...
type pressureUnit = 'pa' | 'kpa' | 'atm' | 'bar' | 'mbar' | 'psi' | 'mmhg' | 'torr';
...
function PressureConverter() {
  const convertPressure = (value: number, unit: string): Record<string, number> => {
    if (!Object.keys(UNIT_TO_PASCALS).includes(unit)) {
      throw new Error(`Invalid unit: ${unit}. Supported units: ${Object.keys(UNIT_TO_PASCALS).join(', ')}`);
    }
