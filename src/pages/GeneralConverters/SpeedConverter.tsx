...
function SpeedConverter() {
  const convertSpeed = (value: number, unit: string): Record<string, number> => {
    if (!Object.keys(UNIT_TO_METERS_PER_SECOND).includes(unit)) {
      throw new Error(`Invalid unit: ${unit}. Supported units: ${Object.keys(UNIT_TO_METERS_PER_SECOND).join(', ')}`);
    }

    // Convert to intermediate unit (meters per second)
    const intermediateValue = value * UNIT_TO_METERS_PER_SECOND[unit as SpeedUnit];
