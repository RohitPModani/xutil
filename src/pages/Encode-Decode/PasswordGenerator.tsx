...
function calculatePasswordStrength(
  ...
  const { includeNumbers, includeSpecial, includeUppercase, includeLowercase } =
    options;
  let score = 0;
...
function generatePassword(
  ...
  if (includeNumbers)
    requiredChars.push(
      DIGITS[
        Math.floor(
          crypto.getRandomValues(new Uint32Array(1))[0] % DIGITS.length
        )
      ]
    );
  if (includeSpecial)
    requiredChars.push(
      PUNCTUATION[
        Math.floor(
          crypto.getRandomValues(new Uint32Array(1))[0] % PUNCTUATION.length
        )
      ]
    );
  if (includeLowercase)
    requiredChars.push(
      ASCII_LOWERCASE[
        Math.floor(
          crypto.getRandomValues(new Uint32Array(1))[0] % ASCII_LOWERCASE.length
        )
      ]
    );
  if (includeUppercase)
    requiredChars.push(
      ASCII_UPPERCASE[
        Math.floor(
          crypto.getRandomValues(new Uint32Array(1))[0] % ASCII_UPPERCASE.length
        )
      ]
    );
  ...
  if (remainingLength < 0) {
    throw new Error(
      `Password length (${length}) is too short to include all required character types. Minimum length for current selection is ${requiredChars.length}.`
    );
  }
...
function PasswordGenerator() {
  ...
  const fetchPassword = useCallback(() => {
    ...
    try {
      ...
      setError(
        err.message ||
          "Failed to generate password. Please check your settings."
      );
    } finally {
      setIsLoading(false);
    }
      ...
      <div className="max-w-6xl mx-auto px-4 py-4 sm:py-8 section">
        ...
        <SectionCard>
          <div className="space-y-4">
            ...
            <ErrorBox
              message={error}
              id={error ? "password-error" : undefined}
            />
          </div>
        </SectionCard>
