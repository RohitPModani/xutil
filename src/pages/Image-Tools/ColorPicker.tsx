...
function ColorPicker() {
  ...
  const validateHslComponent = (value: string, type: 'h' | 's' | 'l'): boolean => {
    if (value === '') return true;
  ...
  const handleRgbChange = useCallback((e: React.ChangeEvent<HTMLInputElement>, component: 'r' | 'g' | 'b') => {
    ...
    if (!validateRgbComponent(r) || !validateRgbComponent(g) || !validateRgbComponent(b)) {
      if ((r && parseInt(r, 10) > 255) || (g && parseInt(g, 10) > 255) || (b && parseInt(b, 10) > 255)) {
        setError('RGB values must be between 0 and 255');
      } else if ((r && isNaN(parseInt(r, 10))) || (g && isNaN(parseInt(g, 10))) || (b && isNaN(parseInt(b, 10)))) {
  ...
  const handleHslChange = useCallback((e: React.ChangeEvent<HTMLInputElement>, component: 'h' | 's' | 'l') => {
    let value = e.target.value.replace(/[^0-9]/g, '');
    ...
    if (!validateHslComponent(h, 'h') || !validateHslComponent(s, 's') || !validateHslComponent(l, 'l')) {
      if ((h && parseInt(h, 10) > 360) || (s && parseInt(s, 10) > 100) || (l && parseInt(l, 10) > 100)) {
        setError(`Invalid HSL values (H: 0-360, S/L: 0-100)`);
      } else if ((h && isNaN(parseInt(h, 10))) || (s && isNaN(parseInt(s, 10))) || (l && isNaN(parseInt(l, 10)))) {
      ...
      <div className="max-w-3xl mx-auto px-4 py-4 sm:py-8 section">
        ...
        <SectionCard className="mb-4">
          <div className="flex items-center justify-between mb-4">
            ...
            <ClearButton
              onClick={handleClear}
              disabled={!color && !hexInput && !rgbInput.r && !rgbInput.g && !rgbInput.b && !hslInput.h && !hslInput.s && !hslInput.l}
